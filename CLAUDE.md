...
| `app.h` | ~225 | All shared types: `struct session` (incl. `thread_id`), `struct window_context`, `struct console_metrics`, `struct preferences` |
...
| `shell.c` | ~3600 | Local shell: 30+ commands (ls, cd, cat, cp, mv, rm, mkdir, ps, free, df, etc.), tab completion, history |
...
Custom bitmap font (`symbolfont.r`) for Unicode characters that don't exist in Mac Roman: box drawing (U+2500-U+257F), block elements (U+2580-U+259F), geometric shapes, and some CP437 characters. Generated by `tools/generate_symbolfont.py` at 7 sizes (9, 10, 12, 14, 18, 24, 36pt) with metrics matched to Monaco. The renderer switches fonts mid-draw-run when a symbol character is encountered (flagged in scrollback attrs bit 4).
...
- **Symbol font rendering**: `symbolfont.r` defines NFNT resources at IDs 20000-20006. Font family FOND at ID 20000, name "STSymbol". Glyph metrics must exactly match Monaco at each size for seamless tiling
